...
public class UsbDeviceScanner
{
    ...
    public void SaveToCsv(List<UsbDeviceInfo> devices, string filePath, string storeName, string storeId,string stationName)
    {
        using (var writer = new StreamWriter(filePath))
        {
            writer.WriteLine("StoreName,StoreId");
            writer.WriteLine($"\"{storeName}\",\"{storeId}\"");    
            writer.WriteLine("StationName,VendorName,ProductName,VendorId,ProductId,SerialNumber");

            foreach (var device in devices)
            {
                writer.WriteLine($"\"{stationName}\",\"{device.VendorName}\",\"{device.ProductName}\",{device.VendorId},{device.ProductId},\"{device.SerialNumber}\"");
            }
        }
